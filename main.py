...
def print_startup_banner(host: str, port: int) -> None:
    ...
    def sanitize_console_text(text: str) -> str:
        """
        Convert banner text to a form safe for legacy terminal encodings.

        Args:
            text: Banner line to print.

        Returns:
            Terminal-safe text that preserves content on non-UTF consoles.
        """
        ...
        normalized_text = text.translate(replacements)
        return normalized_text.encode(console_encoding, errors="replace").decode(
            console_encoding,
            errors="replace",
        )
    ...
    def safe_console_print(text: str = "") -> None:
        """
        Print banner text with fallback for terminals that cannot encode Unicode.
        ...
        try:
            ...
            print(sanitize_console_text(text))
    ...
    safe_console_print()
    safe_console_print(f"  {WHITE}{BOLD}👻 {APP_TITLE} v{APP_VERSION}{RESET}")
    safe_console_print()
    safe_console_print(f"  {WHITE}Server running at:{RESET}")
    safe_console_print(f"  {GREEN}{BOLD}➜  {url}{RESET}")
    safe_console_print()
    safe_console_print(f"  {DIM}API Docs:      {url}/docs{RESET}")
    safe_console_print(f"  {DIM}Health Check:  {url}/health{RESET}")
    safe_console_print()
    safe_console_print(f"  {DIM}{'─' * 48}{RESET}")
    safe_console_print(f"  {WHITE}💬 Found a bug? Need help? Have questions?{RESET}")
    safe_console_print(f"  {YELLOW}➜  https://github.com/jwadow/kiro-gateway/issues{RESET}")
    safe_console_print(f"  {DIM}{'─' * 48}{RESET}")
    safe_console_print()
